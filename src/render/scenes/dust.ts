...
export class DustScene implements LayerScene {
  ...
  update(s: GameState, _m: Mults, dt: number, time: number): void {
    ...
    // Orbitkörper: sichtbar ab Kauf — die Auto-Zündung ist seit dem Trickle reset-frei,
    // die frühere 5-s-Nachleuchte ist überflüssig (Fade übernimmt die Opacity-Glättung)
    for (let t = 0; t < 8; t++) {
